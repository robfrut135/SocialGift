...
public class OAuth2SecurityConfiguration {
	...
	protected static class ResourceServer extends ResourceServerConfigurerAdapter {
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			http
			.authorizeRequests()
				.antMatchers(OauthSvcApi.TOKEN_PATH).anonymous();
			...
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/gift/**")
				.access("#oauth2.hasScope('read')");		
			http
			.authorizeRequests()
				.antMatchers("/gift/**")
			...
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/chain/**")
				.access("#oauth2.hasScope('read')");		
			http
			.authorizeRequests()
				.antMatchers("/chain/**")
	...
	protected static class OAuth2Config extends AuthorizationServerConfigurerAdapter {
		...
		public OAuth2Config() throws Exception {			
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
					...
					.scopes("read","write").resourceIds("gift", "flag", "chain", "user")
					.and()					
					
					.withClient("mobileReader").authorizedGrantTypes("password")
					.authorities("ROLE_CLIENT").resourceIds("gift", "flag", "chain", "user")
					.scopes("read")
			...
			combinedService_ = new ClientAndUserDetailsService(csvc);
		}
